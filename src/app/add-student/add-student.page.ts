...
submitForm(){
 ...
 this.http.post('http://localhost:3000/addingStudents/student', body).subscribe((response) => {
  ...
  this.sendEmail(data.createdStudent.email , 'your credencials', data.createdStudent.password);
...
sendEmail( toEmail: any, subject: any, text: any){
  ...
  this.http.post('http://localhost:3000/addingProfessors/send-email', body).subscribe((response) => {
    console.log(response);
  })
}
}